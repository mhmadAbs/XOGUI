            ...
            if len(p_id) > 0:
                with open("../Backup/Results.txt", "r") as f:
                    ...
                    for each_line in f.readlines():
                        if each_line.startswith("Mohamad"):
        ...
        def playAi():
            ...
            if len(squares_strings) > 0 and not checkResult():  # There is at least one free place
                ...
                if checkResult():
                    ...
                    nonlocal ai_wins
                    ai_wins += 1
                    ai_score_lbl.config(text=str(ai_score_lbl["text"]))

            for btn in btns:  # To prevent player from selecting a position that is full
                if btn["text"] != "":
                    btn.config(state="disabled")

        def updateResult(id_):

            with open("../Backup/Results.txt", "a") as f:
                f.write("{}-{},AI-{}\n".format(id_, player_wins, ai_wins))

        def exit_f():
            updateResult(id_entry.get())
            root.destroy()
